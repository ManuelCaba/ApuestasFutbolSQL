...
EXECUTE sys.sp_addmessage @msgnum = 50003, @severity = 16, @msgtext = N'The allowed quantity has been exceeded', @lang = 'us_english', @replace = 'replace';
EXECUTE sys.sp_addmessage @msgnum = 50003, @severity = 16, @msgtext = N'Se ha sobrepasado la cantidad permitida', @lang = 'spanish', @replace = 'replace';

EXECUTE sys.sp_addmessage @msgnum = 50004, @severity = 16, @msgtext = N'You have already made that bet before', @lang = 'us_english', @replace = 'replace';
EXECUTE sys.sp_addmessage @msgnum = 50004, @severity = 16, @msgtext = N'Ya ha realizado esa apuesta anteriormente', @lang = 'spanish', @replace = 'replace';
GO
...
GO
CREATE OR ALTER TRIGGER LimiteDineroApostado ON Apuestas AFTER INSERT, UPDATE
AS
	BEGIN
		IF (EXISTS (
					SELECT IDPartido FROM Apuestas
					WHERE IDPartido IN (SELECT IDPartido FROM Inserted)
					GROUP BY IDPartido
					HAVING SUM(DineroApostado) > 5000
					))
			BEGIN
				DECLARE @ErrorCantidadApostada NVarchar(255) = FormatMessage(50003);
				THROW 50003, @ErrorCantidadApostada, 1
